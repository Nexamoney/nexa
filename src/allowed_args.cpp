...
// Copyright (c) 2017-2026 The Bitcoin Unlimited developers
...
static void addChainSelectionOptions(AllowedArgs &allowedArgs)
{
    allowedArgs.addHeader(_("Chain selection options:"))
        ...
        .addArg("stormtest", optionalBool, _("Use the stormtest chain"))
...
static void addGeneralOptions(AllowedArgs &allowedArgs, HelpMessageMode mode)
{
    ...
    allowedArgs.addArg("loadblock=<file>", requiredStr, _("Imports blocks from external blk000??.dat file on startup"))
        ...
        .addArg("resync", optionalBool, _("Entirely resyncronize the blockchain database from genesis"))
...
static void addTweaks(AllowedArgs &allowedArgs, CTweakMap *pTweaks)
{
    ...
    for (i = pTweaks->begin(); i != pTweaks->end(); ++i)
    {
        ...
        std::set<std::string> hiddenTweaks = {"mining.xval", "net.blockDownloadWindow", "net.blockRetryInterval",
            ...
            "consensus.fork1Time", "consensus.fork2Time", "test.maxReorgDepth", "cache.maxHeaders",
            "test.requestMissingSubblocks"};
