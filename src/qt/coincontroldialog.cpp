...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog)
{
    ...
    Q_FOREACH (const CAmount &amount, CoinControlDialog::payAmounts)
    {
        ...
        if (amount > 0)
        {
            CTxOut txout(amount, (CScript)std::vector<unsigned char>(24, 0), CTxOut::SATOSCRIPT);
    ...
    if (nQuantity > 0)
    {
        ...
        if (nPayAmount > 0)
        {
            ...
            {
                CTxOut txout(nChange, (CScript)std::vector<unsigned char>(24, 0), CTxOut::SATOSCRIPT);
                if (txout.IsDust())
                {
    ...
    for (const auto &coins : mapCoins)
    {
        ...
        for (const COutput &out : coins.second)
        {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show address. In tree mode,
