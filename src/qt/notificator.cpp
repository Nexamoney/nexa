...
Notificator::Notificator(const QString &_programName, QSystemTrayIcon *_trayicon, QWidget *_parent)
    : QObject(_parent), parent(_parent), programName(_programName), mode(None), trayIcon(_trayicon)
#ifdef USE_DBUS
      ,
      interface(0)
#endif
...
FreedesktopImage::FreedesktopImage(const QImage &img)
    : width(img.width()), height(img.height()), stride(img.width() * BYTES_PER_PIXEL), hasAlpha(true),
      channels(CHANNELS), bitsPerSample(BITS_PER_SAMPLE)
{
...
int FreedesktopImage::metaType() { return qDBusRegisterMetaType<FreedesktopImage>(); }
...
void Notificator::notifyGrowl(Class cls, const QString &title, const QString &text, const QIcon &icon)
{
    const QString script(
        ...
        "  register as application \"%1\" all notifications allNotificationsList default notifications "
    ...
    MacNotificationHandler::instance()->sendAppleScript(
