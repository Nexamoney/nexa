...
UniValue addnode(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2 || (strCommand != "onetry" && strCommand != "add" && strCommand != "remove"))
        throw runtime_error("addnode \"node\" \"add|remove|onetry\"\n"
                            ...
                            "There is maxiumum (net.maxOutboundConnections) number of outbound connections allowed "
                            "however addnode allows you to exceed this by the same number. So if your maximum outbound "
                            "is 8 then you can connect up to an additonal 8, giving you a total of 16 outbound "
                            "connections. Another point to condsider is while you can add as many addnodes "
                            "to the list as you want via this rpc command only up to 8 of them will actually make a "
                            "connection, the rest will remain unused unless one of the other nodes in the list "
                            "disconnects and makes a slot available."
                            "\nArguments:\n"
                            ...
                            "\nExamples:\n" +
                            HelpExampleCli("addnode", "\"192.168.0.6:7228\" \"onetry\"") +
                            HelpExampleRpc("addnode", "\"192.168.0.6:7228\", \"onetry\""));
...
UniValue disconnectnode(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error("disconnectnode \"node\" \n"
                            ...
                            "1. \"node\"     (string, required) The node (see getpeerinfo for nodes)\n"
                            "\nExamples:\n" +
                            HelpExampleCli("disconnectnode", "\"192.168.0.6:7228\"") +
                            HelpExampleRpc("disconnectnode", "\"192.168.0.6:7228\""));

    CNodeRef node = FindNodeRef(params[0].get_str());
    if (!node)
        throw JSONRPCError(RPC_CLIENT_NODE_NOT_CONNECTED, "Node not found in connected nodes");

    node->CloseSocketDisconnect("disconnectnode RPC called");
...
UniValue getaddednodeinfo(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "    \"addresses\" : [\n"
            "       {\n"
            "         \"address\" : \"192.168.0.201:7228\",  (string) The  server host and port\n"
