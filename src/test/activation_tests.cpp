...
BOOST_AUTO_TEST_CASE(isfork1enabled)
{
    ...
    BOOST_CHECK(!IsUpgrade2Activated(&blocks.back()));

    SetMTP(blocks, activation);
    BOOST_CHECK(!IsUpgrade2Activated(&blocks.back()));
    BOOST_CHECK(IsUpgrade2Pending(&blocks.back()));

    SetMTP(blocks, activation + 1);
    BOOST_CHECK(IsUpgrade2Activated(&blocks.back()));
    BOOST_CHECK(!IsUpgrade2Pending(&blocks.back()));
    BOOST_CHECK(!(IsUpgrade2Activated(&blocks.back()) && !(IsUpgrade2Activated(&blocks.end()[-2]))));
}
