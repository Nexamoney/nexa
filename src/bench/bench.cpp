...
void benchmark::ConsolePrinter::result(const State &state)
{
    ...
    std::cout << state.m_name << ", " << state.m_num_evals << ", " << state.m_num_iters << ", " << total << ", "
              << front << ", " << back << ", " << median << std::endl;
}
...
void benchmark::PlotlyPrinter::header()
{
    std::cout << "<html><head>"
              ...
              << "</head><body><div id=\"myDiv\" style=\"width:" << m_width << "px; height:" << m_height
              << "px\"></div>"
...
{
    std::cout << "{ " << std::endl << "  name: '" << state.m_name << "', " << std::endl << "  y: [";
    ...
    std::cout << "]," << std::endl
              << "  boxpoints: 'all', jitter: 0.3, pointpos: 0, type: 'box'," << std::endl
              << "}," << std::endl;
}
