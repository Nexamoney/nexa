...
#ifndef NEXA_BENCH_BENCH_H
#define NEXA_BENCH_BENCH_H
...
public:
    ...
    State(std::string name, uint64_t num_evals, double num_iters, Printer &printer)
        : m_name(name), m_num_iters_left(0), m_num_iters(num_iters), m_num_evals(num_evals)
    {
    }
...
public:
    ...
    static void RunAll(Printer &printer,
        uint64_t num_evals,
        double scaling,
...
// Choose a num_iters_for_one_second that takes roughly 1 second. The goal is that all benchmarks should take
// approximately
