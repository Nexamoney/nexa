...
class AbandonConflictTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        sync_blocks(self.nodes)
        waitFor(waitTime,lambda: balance - self.nodes[0].getbalance() < Decimal("10")) #no more than fees lost
        balance = self.nodes[0].getbalance()
        ...
        waitFor(waitTime, lambda: self.nodes[0].getbalance() == balance - Decimal("3000000") + Decimal("2499960"))
        balance = self.nodes[0].getbalance()
       ...
       # Verify txs no longer in txpool
        ...
        waitFor(waitTime, lambda: self.nodes[0].getbalance() == balance + Decimal("3000000"))
        balance = self.nodes[0].getbalance()
        ...
        waitFor(waitTime, lambda: self.nodes[0].getbalance() == balance - Decimal("2000000") + Decimal("1499998"))
        balance = self.nodes[0].getbalance()

        # Send child tx again so its unabandoned
        self.nodes[0].enqueuerawtransaction(signed2["hex"],"flush")
        waitFor(waitTime, lambda: self.nodes[0].getbalance()== balance - Decimal("1000000") - Decimal("1499998") + Decimal("2499960"))
        balance = self.nodes[0].getbalance()
